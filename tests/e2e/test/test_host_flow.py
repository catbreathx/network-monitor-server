...
class TestHostFlow:
    def test_host_flow(self, test_user: schema.User):
        ...
        response = api_commands.create_host(host_create, access_token)
        response = api_commands.get_one_host(response.json()["id"], access_token)
        host1 = response.json()
        ...
        response = api_commands.update_host(host_response["id"], host_update, access_token)

        assert_success_response(response, HTTPStatus.OK)

        response = api_commands.get_one_host(host_response["id"], access_token)
        assert response.status_code == HTTPStatus.OK
        host2 = response.json()
        assert_model_response(host_update, host2)

        response = api_commands.get_all_hosts(access_token)
        expected_hosts = [host1, host2]
        assert response.json() == expected_hosts

        unknown_resource_id = 1000
        response = api_commands.get_one_host(unknown_resource_id, access_token)
    ...
    def test_when_unauthorized(self, test_user: schema.User):
        ...
        response = api_commands.create_host(host_create, jwt_token)
